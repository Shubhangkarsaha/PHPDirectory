...
if(isset($_GET['DAMID']))
  {
    $dam_id 		= $_GET['DAMID'];
	$dam_boro	 	= $_GET['Office'];
	
	$asql = "Select * from duty_allotment_master where dam_id ='$dam_id' and dam_borough_id ='$dam_boro' and dam_status > '0'";
  ...
  if($_POST['Submit']=="Allot")
	{
		...
		$upsql = "update duty_allotment_master set dam_status = '0' where dam_id = '$dam_id'";
		$uprec = q($upsql);
		if($uprec>0)
		  {
			$sql = "insert into `duty_allotment_master`(dam_emp_id, dam_vehicle, dam_log_book, dam_ward, dam_borough_id, dam_fuel_type, dam_trips, dam_fuel_per_trip, dam_update_date, dam_update_by)values('$dam_emp_id', '$dam_vehicle', '$dam_log_book', '$dam_ward', '$dam_borough', '$dam_fuel_type', '$dam_trips', '$dam_fuel_per_trip', '$today', '$_SESSION[lid]')";
			//echo $sql;exit;
					
		$rec = q($sql);
					
		if($rec>0)
		   echo alert_location_replace('Allotment Details updated successfully.','office_wise_vehicle_list.php?Office='.$dam_boro);
		else
		   echo error_alert_location_replace('office_wise_vehicle_list.php?Office='.$dam_boro);
          }	
    }
}
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
          <table width="98%" border="0" align="center" cellpadding="1" cellspacing="1" class="main_bod" >
            ...
            <tr class="fnt">
              ...
              <td width="14%" height="39" align="left"><select name="dam_vehicle" id="dam_vehicle" required="required">
                ...
                <?php 
							...
							while($vres = f($vrec)){
						?>
                <option value="<?php echo $vres['vm_id'];?>"<?php if(isset($_GET['DAMID'])){if($vres['vm_id'] == $ares['dam_vehicle']){echo "selected ";}else{echo "disabled";}}?>><?php echo $vres['vm_name'];?></option>
                <?php }?>
              </select></td>
              ...
              <td width="20%" height="38" align="left"><select name="dam_emp_id" id="dam_emp_id" required="required">
                ...
                <?php 
						...
						?>
                <option value="<?php echo $emp_res['emp_id'];?>"<?php if(isset($_GET['DAMID'])){if($emp_res['emp_id'] == $ares['dam_emp_id']){echo "selected";}else{echo "disabled";}}?>><?php echo $emp_res['emp_name'];?></option>
                <?php }?>
              </select></td>
            </tr>
            <tr class="fnt">
            ...
            <tr>
              <td height="35" colspan="9" align="center"><input class="head_font" type="submit" name="Submit" <?php if(isset($_GET['DAMID'])) { echo "value=Allot" ;} else { echo "value=Submit" ;}?> />
